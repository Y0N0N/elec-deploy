"""
生成电网约束/机组状态聚合因子 (Grid Constraint Factors v1) — deploy 版
数据源: 披露矩阵 (cfg.DISCLOSURE_MATRIX) 中 5540+ 个机组约束文件
...
移植自 Yonon/50_tool_grid_constraint_factors_v1.py：
  - 路径改用 deploy/_cfg（不再硬编码 /root）
  - 日期范围尊重环境变量 DEPLOY_FACTOR_START/END（由 2A_rebuild 注入）
"""
import os, glob, gc, sys, warnings
...
print(f"  GC 因子总数: {len(all_gc)}, 泄露检查: {'警告: ' + str(leaky) if leaky else '完成 无泄露'}")
