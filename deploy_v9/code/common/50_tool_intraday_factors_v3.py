"""
生成安全的日内横向因子 (Intraday Factors v2) — deploy 版
...
移植自 Yonon/50_tool_intraday_factors_v3.py：
...
# 日期范围（由 2A_rebuild 注入；未注入则自动从数据推断）
def _date_range():
...
MATRIX_PATH = cfg.DISCLOSURE_MATRIX
DATASET_PATH = cfg.FACTOR_DIR
...
price_df   = pd.read_feather(f"{cfg.ACTUAL_MATRIX}/日前统一结算价.feather")
load_df    = pd.read_feather(f"{cfg.ACTUAL_MATRIX}/实际用电量.feather")
trade_df   = pd.read_feather(f"{cfg.ACTUAL_MATRIX}/日前成交电量.feather")
rt_price_df = pd.read_feather(f"{cfg.ACTUAL_MATRIX}/实时统一结算价.feather")
...
# 预测数据
py_path = MATRIX_PATH
feather_files = [os.path.join(py_path, f) for f in os.listdir(py_path) if f.endswith('.feather')]
...
for datafield in new_data_field:
    madata_path = os.path.join(py_path, f"{datafield}.feather")
...
for name, series in intraday_factors.items():
    ...
    df_wide.to_feather(os.path.join(DATASET_PATH, f"{name}.fea"))
    saved += 1
...
if leaky:
    print(f"  警告: 警告: 仍有价格泄露因子: {leaky}")
else:
    print(f"  完成 无价格泄露因子")

# 验证预测集可用性
print(f"\n  预测日期 ({FACTOR_END} 附近) 的空值检查:")
for f in h_feas[:5]:
    df_test = pd.read_feather(os.path.join(DATASET_PATH, f))
