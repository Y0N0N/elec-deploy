...
for _p in (_HERE, _ROOT, os.path.join(_ROOT, 'code', 'common')):
...
# 复用 deploy_v9 的训练模块（方向头/量级头/数据加载/小时先验，v9 训练脚本同目录）
_spec = importlib.util.spec_from_file_location(
    'v9train', os.path.join(_HERE, '31_model_v9_direction_run.py'))
...
def main():
    ...
    ap.add_argument('--out-dir', default=os.path.join(_ROOT, 'models'))
    args = ap.parse_args()
    ...
    # 更新 latest_model.json 指针（BUG-2 修复：v9.1 训练后写入指针，
    # 避免指针文件永远缺 v9.1 条目；与 v9 训练脚本行为对齐）
    ptr = {}
    if os.path.exists(v9t.cfg.LATEST_MODEL_FILE):
        try:
            ptr = json.load(open(v9t.cfg.LATEST_MODEL_FILE))
        except Exception:
            pass
    ptr['v9.1'] = save_path
    json.dump(ptr, open(v9t.cfg.LATEST_MODEL_FILE, 'w'), ensure_ascii=False, indent=2)
    print(f"  最新模型指针已更新: {v9t.cfg.LATEST_MODEL_FILE}")
