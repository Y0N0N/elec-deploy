...
def group_return_ana(pred: pd.DataFrame | pd.Series, y_true: pd.Series, n: int = 10, groupby: str = "time",
                     figsize: tuple = (10, 6), pred_column: str = "predict") -> None:
    """
    ...
    predict = predict.sort_values(pred_column, ascending=False)
    acc = accuracy(predict[pred_column], predict["label"], sign=">=")
...
def single_factor_ana(feature: pd.Series):
    ...
    ax0.set_ylabel("count", fontsize=16)  # 设置 y 轴标签字体大小
    ax0.tick_params(axis='both', labelsize=14)  # 设置刻度字体大小

    ax1.plot(mean, label="mean")
    ax1.set_ylabel("mean", fontsize=16)  # 设置 y 轴标签字体大小
    ax1.legend(fontsize=14)  # 设置图例字体大小
    ax1.tick_params(axis='both', labelsize=14)  # 设置刻度字体大小
    ax11 = ax1.twinx()
    ax11.plot(std, color="orange", label="std")
    ax11.set_ylabel("std", fontsize=16)  # 设置 y 轴标签字体大小
    ax11.tick_params(axis='y', labelsize=14)  # 设置双轴刻度字体大小

    h1, l1 = ax1.get_legend_handles_labels()
    h2, l2 = ax11.get_legend_handles_labels()
    ax1.legend().set_visible(False)  # 隐藏第一个图例
    ax11.legend(h1 + h2, l1 + l2, fontsize=14)  # 设置合并后的图例字体大小
    ...
    ax0.set_title(feature.name, fontsize=18)

    plt.subplots_adjust(hspace=0.3)
    plt.show()


def calc_fitness_metrics(pred: pd.Series, y: pd.Series, plot_rolling_mse: bool = False, window: int = 20):
    """
    计算预测性能指标并可选择性地绘制滚动时间窗口MSE变化

    参数:
    pred: 预测值，pd.Series，索引为(datetime, instrument)
    y: 真实值，pd.Series，索引为(datetime, instrument)
    plot_rolling_mse: 是否绘制滚动时间窗口MSE变化图，默认为False
    window: 滚动窗口大小，默认为20
    ...
    """
    # 确保pred和y有相同的索引
    common_idx = pred.index.intersection(y.index)
    ...
    # 计算QLIKE (Quasi-Likelihood)
    # 确保预测值为正数，避免对数计算出错
    pred_positive = np.maximum(pred, 1e-4)
    qlike = np.mean(y / pred_positive - np.log(y / pred_positive) - 1)

    # 绘制滚动时间窗口MSE变化
    if plot_rolling_mse:
        # 确保索引包含datetime和instrument
        if not (isinstance(pred.index, pd.MultiIndex) and len(pred.index.names) == 2):
            raise ValueError("Input series must have MultiIndex with ('datetime', 'instrument')")

        # 计算每个时间点的MSE
        squared_errors = (pred - y) ** 2
        daily_mse = squared_errors.groupby(level=0).mean()

        # 按时间排序
        daily_mse = daily_mse.sort_index()

        # 计算滚动MSE
        rolling_mse = daily_mse.rolling(window=window).mean()

        # 绘制滚动MSE变化图
        plt.figure(figsize=(12, 6))
        plt.plot(rolling_mse.index, rolling_mse.values, 'b-', linewidth=2)
        plt.title(f'Rolling MSE (Window Size: {window})')
        plt.xlabel('Date')
        plt.ylabel('Mean Squared Error')
        plt.grid(True, linestyle='--', alpha=0.7)

        # 添加平均MSE水平线
        plt.axhline(y=mse, color='r', linestyle='--', label=f'Average MSE: {mse:.6f}')
        plt.legend()
