"""
Pipeline v7.2 — T112-C2: gas_limit 族事件因子增量验证

背景: 全量 68 个事件因子验证不佳 (凌晨漏报 9.1%→10.9%),
      其他事件类别 (outage/voltage/dispatch) 引入噪声, 稀释气量管控信号。
本实验: 与 scarcity 脚本 **同协议** (每周重训 + 稀缺权重 + 同种子),
  仅追加 9 个 ev_gas_limit_* 因子 (气量管控族),
  验证核心前兆信号是否有增量。

红线: 同 walk-forward — 评估日 d 绝不进 train/valid; ev_* 因子滚动基线只用过去, 无未来泄露。

产物: data/walkforward_v7.2_preds_ev.csv + data/ev_compare.json + show/v7/report_ev_gas.png
"""
...
print("=" * 72)
print("  T112-C2: gas_limit 族事件因子增量验证")
print(f"  369 特征 + 9 gas_limit 因子 | 稀缺权重 {W_SCALE}x/凌晨{W_MORNING}x | "
      f"回测 {BT_START}~{BT_END} | n_est={N_EST} | n_jobs={JOBS}")
print("=" * 72)

# ============ 加载特征 (369 + 9 gas_limit ev) ============
print("\n[Phase 0] 加载新鲜集 369 + gas_limit 事件因子...", flush=True)
t0 = time.time()
...
ev_names = sorted([n[:-4] for n in os.listdir(dataset_path)
                   ...
                   and n.endswith('.fea')])
print(f"  gas_limit 事件因子: {len(ev_names)} 个: {ev_names}", flush=True)
...
m_ev = miss_metrics(ev, 'gas_limit族')
...
try:
    ...
    ax.bar(x + w/2, ev_v, w, label='gas_limit族', color='#e8a06a')
    ax.set_xticks(x); ax.set_xticklabels(cats); ax.set_ylabel('%')
    ax.set_title('T112-C2: gas_limit 事件因子 vs 稀缺基线 (同 walk-forward 协议)')
    ax.legend(); ax.grid(alpha=0.3, axis='y')
    plt.tight_layout()
    plt.savefig(f'{YONON_PATH}/show/v7/report_ev_gas.png', dpi=150); plt.close()
    print(f"  图已保存: {YONON_PATH}/show/v7/report_ev_gas.png")
except Exception as e:
    print(f"  图跳过: {e}")

print("\n" + "=" * 72)
print("  T112-C2 gas_limit 事件因子增量验证完成")
print("=" * 72)
